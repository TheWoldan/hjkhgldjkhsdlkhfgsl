...
-- 📍 RETURN TP TOGGLE (BAĞIMSIZ)
local returnTpEnabled = false
local savedCFrame = nil
local targetCFrame = CFrame.new(-34.60, 5.39, 1959.33)

Tabs.Killer:AddToggle("ReturnTP", {
    Title = "Tiny Island TP",
    Description = "Teleport to the target when enabled, return to the previous position when disabled.",
    Default = false
}):OnChanged(function(Value)
    returnTpEnabled = Value

    local lp = game:GetService("Players").LocalPlayer
    local char = lp.Character or lp.CharacterAdded:Wait()
    local hrp = char:WaitForChild("HumanoidRootPart")

    if returnTpEnabled then
        -- 📌 MEVCUT KONUMU KAYDET
        savedCFrame = hrp.CFrame

        -- ⏱️ 0.1 sn sonra hedefe ışınla
        task.delay(0.1, function()
            if returnTpEnabled and hrp and hrp.Parent then
                hrp.CFrame = targetCFrame
            end
        end)
    else
        -- 🔙 ESKİ KONUMA GERİ DÖN
